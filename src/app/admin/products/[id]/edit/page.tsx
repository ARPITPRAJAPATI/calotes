  ...
  const fetchCategoriesAndProduct = async () => {
    try {
      ...
      if (prodRes.ok) {
        ...
        setCompareAtPrice(prodData.compareAtPrice ? prodData.compareAtPrice.toString() : '');
        setImages(prodData.images || []);
        setCategory(prodData.category?._id || prodData.category || '');
        setBrand(prodData.brand || 'Vintage');
        setCondition(prodData.condition || 'Great');
        setSelectedSizes(prodData.sizes || []);
        setSku(prodData.sku || '');
        setStock(prodData.stock.toString() || '1');
        setIsFeatured(prodData.isFeatured || false);
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            >
              {isSubmitting ? 'Updating Piece...' : 'Update Piece'}
            </button>
          </div>
        </div>
      </form>
