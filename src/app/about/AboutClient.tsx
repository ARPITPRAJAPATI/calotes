    ...
    <div className="w-full pt-28 md:pt-36 pb-24">
      ...
      <section className="px-6 md:px-12 max-w-[1400px] mx-auto mb-24 md:mb-40">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-14 md:gap-24">
          ...
          >
            {/* If there are extra brand images, show them as a secondary collage */}
            {brandImages.length > 2 ? (
              <div className="grid grid-cols-2 gap-3 mb-8">
                {brandImages.slice(2, 6).map((img, i) => (
                  <div key={i} className="relative aspect-square overflow-hidden bg-bg-warm border border-border">
      ...
      {/* ── Philosophy image (first brand image as full-width accent if >2 images) */}
      {brandImages.length >= 2 && (
        <section className="px-6 md:px-12 max-w-[1800px] mx-auto mb-24 md:mb-40">
          <div className="relative aspect-[21/9] overflow-hidden bg-bg-warm border border-border">
            <img
              src={philosophyImage}
              alt="Brand philosophy"
              className="w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-r from-bg/70 to-transparent flex items-center">
              <div className="px-10 md:px-20 max-w-lg">
                <p className="section-label mb-3 text-text/80">The Archive</p>
                <h2 className="font-display font-black text-3xl md:text-5xl uppercase tracking-tighter leading-[0.9] text-text">
                  Every piece<br />
                  <span className="text-terracotta">has a story.</span>
                </h2>
              </div>
            </div>
          </div>
